...
如过和部门是多对多关系可以继承`BaseMixin` 里面不包含 `dept_id`字段
...
class permissionConfig:
    modules = [
        ...
        {"module_key": "xxx", "module_name": "中文描述"},  # 新增数据库模型类的权限控制,不需要Modal,需要和数据库名称对应
    ]
    # 默认 会渲染 访问,查询,新增,编辑,删除,导入,导出 字符权限,如需增加或者减少可以 以下方法
     # 自定义配置权限
...
# 服务基类已经实现了 通用增删改查,根据字段查询所有,分页查询所有等,自动过滤角色数据范围,和操作权限校验. 也可以根据需求重构
```
